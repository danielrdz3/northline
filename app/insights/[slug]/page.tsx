...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> { const { slug } = await params; const insight = insightBySlug.get(slug); if (!insight) return {}; const path = `/insights/${insight.slug}/`; return pageMetadata({ title: insight.seoTitle, description: insight.description, path, type: "article" }); }
