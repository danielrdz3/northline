...
export function ServicePage({ service }: { service: LocationService }) {
  return <main><SiteHeader />
    <section className="serviceHero"><div className="serviceHeroInner"><div><p className="eyebrow">{service.short.toUpperCase()} · MICHIGAN</p><h1>{service.name} in Michigan</h1><p>{service.intro}</p><BookingLink label="BOOK A CONVERSATION" /></div><div className="servicePhoto" style={{ backgroundImage: `linear-gradient(#071a2f18,#071a2f44),url('${service.image}')` }}><span aria-hidden="true">{service.icon}</span></div></div></section>
